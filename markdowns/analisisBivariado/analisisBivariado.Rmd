...
##### Sending data to informatica educativa
...
knitr::knit_hooks$set(send_code_data = function(before,options,name,envir) {
  if (!before && !(identical(globalenv(), envir))) {
    ...
    args <- list(
      ...
      session_id=isolate(r_values$email)
    )
...
```{r,context="data"}
r_values <- reactiveValues(sessionId_=stri_rand_strings(1, 16))
```
...
observe({  
  ...
  r_values$email <- email
  if (!is.na(email)){
    url <- paste(get_api_url("/api/v1/session/"),r_values$sessionId_,sep="")
...
```{r correo, exercise=TRUE,send_code_data=FALSE}
email <-
```
