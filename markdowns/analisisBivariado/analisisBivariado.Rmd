...
knitr::knit_hooks$set(send_code_data = function(before,options,name,envir) {
  if (!before && !(identical(globalenv(), envir))) {
      url <- get_api_url("/api/v1/record/")
    args <- list(
      document="Analisis Roxana",
      q_id=options$label,
...
```{r correlacionPearson, exercise=TRJE, eval=TRUE}
...
### **1. Asociación de variables categóricas**s
...
```{r planteamiento hipótesis, exercise=TRUE,echo=TRUE}
#hipótesis científica: Hay relación lineal entre el puntaje obtenido en la prueba de Cultura General y el promedio acumulado de los estudiantes
