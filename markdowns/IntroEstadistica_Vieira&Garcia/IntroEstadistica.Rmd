...
r <- GET("http://localhost:3500/api/v1/session")
...
observe({  
  ...
  cedula <- strsplit(url,split="=")[[1]][2]
  if (!is.na(cedula)){
    url <- paste('http://localhost:3500/api/v1/session/',sessionId_,sep="")
    body <- paste('{userId:',cedula,sep="")
    body <- paste(body,"}",sep="")
    r <- PUT(url,body =body)
